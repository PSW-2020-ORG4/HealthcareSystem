...
namespace IntegrationAdaptersTests.IntegrationTests
{
    public class DbContextInMemory
    {
        ...
        public DbContextInMemory()
        {
            ...
            var pharmacy = new PharmacySystem { Id = 1, Name = "apoteka-1", ApiKey = "api-1", Url = "http://localhost:8080", GrpcAdress = new GrpcAdress("localhost", 9090), ActionsBenefitsExchangeName = "exchange", ActionsBenefitsSubscribed = true };
            _context.Add(pharmacy);
            _context.Add(new ActionBenefit { Id = 1, PharmacyId = 1, Pharmacy = pharmacy, Message = new ActionBenefitMessage("s", "m"), IsPublic = false });
            _context.Add(new ActionBenefit { Id = 2, PharmacyId = 1, Pharmacy = pharmacy, Message = new ActionBenefitMessage("ss", "mm"), IsPublic = true });
